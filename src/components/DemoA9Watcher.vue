...
<template>
  <div id="DemoA9Watcher">
    <fieldset>
      <legend align="left">DemoA9-2: watcher example</legend>
      <p>
        Ask a yes/no question:
        <input v-model="question" />1
        <input v-model="question2" />2
        <input v-model="question3" />3
...
export default {
  ...
  watch: {
    ...
    question(newQuestion) {       //watchdog is a function style1
      if (newQuestion.indexOf("?") > -1) {
        this.getAnswer(newQuestion);
      }
    },
    question2: "getAnswer2",      //watchdog is a function style2
    question3: function(newQuestio){        //watchdog is a function style3
      this.getAnswer3(newQuestio);
    },
    question4: [                  //is defined a array including two callback function in property question4 watcher.
      'getAnswer4',
